...
import { checkedEmail, changedEmail, verifyEmail, checkedCode, checkedNickname, changedNickname, fetchJoin } from '../service/api/userAPI';
...
const Join = () => {
  ...
  return (
    <div>
      <form style={{border: '1px solid black',gap: '10px',padding: '30px', display: 'grid', justifyItems: 'center' }}  onSubmit={handleSubmit}>
        <label htmlFor="email">이메일</label>
        <input type="email" name='email' onChange={emailState} />
        <button type='button' onClick={handleCheckedEmail}>중복확인</button>
        <label htmlFor="code">인증번호</label>
        <input type="text" name='code' onChange={codeState}/>
        <button type='button' onClick={handleCheckedCode}>인증확인</button>
        <label htmlFor="password">비밀번호</label>
        <input type="password" name='password' />
        <label htmlFor="nickname">닉네임</label>
        <input type="text" name='nickname' onChange={nicknameState}/>
        <button onClick={handleCheckedNickname} type='submit'>닉네임 중복확인</button>
